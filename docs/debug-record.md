...
- 第四轮回访补充了一条预览层问题：
  ...
  - 导致视觉上像是影子还停留在老卡
- 第五轮回访补充了一条右键统一拖拽回归：
  - 在“右键按住本体任意位置拖动”的统一链路下
  - 卡内控件从 card 拖出到底板时，松手后仍可能被重新判定回原卡
  - 表现为控件看似拖出，但 `parent` 未切到 `root`
- 第六轮回访补充了一条右键跨卡边界：
  - 从 `A` 卡右键拖入 `B` 卡时，释放点会被偏移，导致落入 `root` 或回落 `A`
  - 复测中可见 `react-flow` 视口在拖动期间被同步平移，平移量与鼠标轨迹一致
- 修复后确认：
...
- 本轮补充右键统一链路修正：
  - 在 `WidgetWrapper` 的右键收尾判定里，增加“原父卡排除”保护
  - 若控件左上角锚点已离开原父卡 bounding box，则不再允许把原父卡当作 drop target
  - 仍保留原有“锚点命中目标 B 卡则进 B 卡；未命中任何卡则落 root”的分支
- 本轮继续补充视口干扰隔离：
  - `WidgetWrapper` 根节点增加 `nopan`，阻断控件拖动时误触发 `ReactFlow` 底板平移
  - 保留空白底板左键平移能力，不影响画布导航
...
- 根因确认：
  ...
  - 右键统一拖拽收尾阶段，原父卡在极端情况下会被误识别为释放目标；即使锚点已离开原卡，仍可能把 `targetContainer` 判回 source parent
  - 右键链路通过 synthetic left mousedown 触发内部拖动时，控件区域未标记 `nopan`，会同时驱动 `ReactFlow` 视口平移，导致收尾锚点与容器坐标系错位
- 实际改动文件清单：
  - `src/builder/WidgetWrapper.tsx`
  - `src/components/KitFactoryBoard.tsx`
  - `src/components/NestedCanvas.tsx`
...
- card A -> card B 预览回归结果：
  ...
  - 实际落地结果保持不变
- 右键统一拖拽回归结果：
  - 使用固定 `3000 + 9333` 链路，通过 CDP websocket 真实浏览器回测 `card -> root`
  - 起点：卡内 `slot_shell`
  - 终点：底板空白区域
  - 结果：`parent` 已从原卡切换为 `root`
  - 调试决策信息显示：`isInsideLatestParent=false` 时，`targetContainerId=null`，随后正确进入 root 落地分支
...
- `2026-04-22`：右键统一拖拽回访中，重新发现 `card -> root` 在松手后偶发误判回原父卡，开始修复
- `2026-04-22`：在 `WidgetWrapper` 增加“锚点已离开原父卡时，禁止把原父卡继续当作目标容器”的保护；通过固定 `3000 + 9333` 的 CDP websocket 真浏览器回测确认 `card -> root` 已恢复
- `2026-04-23`：继续回访 `A -> B` 右键直进，定位到拖动过程触发 `ReactFlow` 视口平移（与鼠标位移同量），导致收尾落点偏移
- `2026-04-23`：`WidgetWrapper` 增加 `nopan` 隔离后，固定 `3000 + 9333` 回归通过：`A -> B` 可直接进入目标卡、`card -> root` 继续可用、空白底板左键平移保持正常
